...
func TestCommitEcosystemCommitsOnlyDirtyRepos(t *testing.T) {
	...
	if n := commitEcosystem(t.Context()); n != 1 {
		t.Errorf("commitEcosystem committed %d repos, want 1 (dirty only)", n)
	}
...
func TestCommitEcosystemNoRoot(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	if n := commitEcosystem(t.Context()); n != 0 {
		t.Errorf("commitEcosystem = %d with no ecosystem dir, want 0", n)
	}
}
