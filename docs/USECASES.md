...
- **Поток [ИИ]:** Python — интерпретатор из apt-list + venv в проекте (оба уровня, F1); базовый `node`
  покрывает TypeScript/React-тулинг (`tsc`, `npm`/`npx`, бандлеры) — отдельный опциональный стек не нужен;
  опциональный каталог (`config/stacks.txt`) — это `dotnet` (печётся при выборе); `go` теперь в обязательной базе как node/python; прочее — ad-hoc,
  `sudo` свободен внутри (I2).
...
```mermaid
flowchart TD
  R([mirabilis]) --> TUI[Go-TUI: запуск/обновить/плагины/харнес/стек/VS Code/секреты/тема]
