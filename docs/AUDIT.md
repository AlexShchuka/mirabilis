...
> **Статус (Phase 2): все находки этого прогона закрыты** — PR #33 (P1 «Согласованность»: H1/H2/H4/M1–M3/M7
> и др.) и follow-up #34 (косметика + остаточные мелочи). Документ оставлен как **провенанс исходного
> состояния** (коммит `13ca73e`), а не как открытый список задач.
...
- **M1** `protect-critical.sh` — advisory hook (фейковый маркер). → честный гейт ([CONSENT.md](CONSENT.md)). **[ИИ] итог:** консент-гейт удалён целиком (не «честный гейт») — защита = граница контейнера (I5), внутри полная свобода (I2).
...
- **M4** `sandbox-context.md` велит менять файл, недоступный агенту и перезаписываемый. → зоны. **[ИИ] итог:** «зоны» неактуальны — gated/файловых зон внутри контейнера нет; sandbox-context.md переписан (граница = контейнер, I5).
...
- **L5** два списка защищённых путей — риск drift. **[ИИ] итог:** неактуально — защиты путей внутри контейнера больше нет.
